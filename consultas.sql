...
--                             |
--                             *
-- ciudad, pais por tienda y el encargado de la misma
-- select store.store_id, store.manager_staff_id ,concat(city.city, ', ', country.country) as ubicacion
-- from country
-- join city on country.country_id = city.country_id
-- join address on address.city_id = city.city_id
...
-- select inventory.store_id, count(*) as numero_ventas
-- from rental
-- join inventory on inventory.inventory_id = rental.inventory_id
-- group by inventory.store_id
-- order by store_id;

-- Nombre encargado por tienda
-- select aux.store_id, aux.ubicacion, concat(staff.first_name, ' ', staff.last_name) as encargado
-- from (
--   select store.store_id, store.manager_staff_id , concat(city.city, ', ', country.country) as ubicacion
--   from country
...
-- Ejercicio 4d
select concat(first_name, ' ', last_name) as nombre_actor, string_agg(distinct category.name, ':') as categorias, string_agg(distinct film.title, ':') as titulos
from actor
